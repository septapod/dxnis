...
### Tools and Methods section — `/tools/` (2026-06-03): BUILT, pending review + deploy

New public resource section that will live at `dxn.is/tools`: a growing library of AI-strategy methods rebuilt as usable tools for credit union and community bank leaders (modeled on Nesta's DIY Toolkit). Files in `tools/`. Source prototype: `~/Library/CloudStorage/Dropbox/Downloads/Recents/ai-evals-field-guide.html`.

- **Flagship entry `tools/evals.html`.** Plain-language adaptation of Hamel Husain's "Your AI Product Needs Evals" (hamel.dev/blog/posts/evals). A teaching page (what-is, the "an eval is just an evaluation" framing, a three-levels accordion, the "look at your data" rule) plus the take-away: a read-only vendor scorecard (8 single-intent questions, each with what a good answer and a dodge sound like) and a cases worksheet. The 8 questions were rebuilt and grounded against Hamel's actual article (he gives no vendor questions; they are the buyer's-side translation, q7-8 are FI additions). Hamel credited in the hero source box and the attribution block; credit union examples labeled illustrative.
- **Index `tools/index.html`.** Card grid: flagship card + one temporary single-entry placeholder (delete at 2+ methods; noted in an HTML comment and the README). Collection name/tagline single-sourced in the `TM` object at the top of `tools/tools.js` (provisional working name "Tools and Methods"; Brent reserves the final name).
- **Chrome reuse, not mimicry.** Real site header/logo/nav/theme-toggle/footer via `../css/styles.css` + `../css/dsl-tokens.css`. Section styles in `tools/tools.css`, namespaced `.tm-`, theme-aware (dark default + light). `tools/tools.js` replicates the theme + nav behavior on the shared `localStorage['theme-preference']` (default dark) rather than loading homepage `js/main.js` (which is homepage-specific).
- **Brand-true to the Swiss grid (passed `.impeccable.md` audit).** Square corners, no side-stripe borders (BAN 1), no nested cards, 8px spacing, gold 2px focus. Content namespaced so the homepage's `.section` scroll-reveal can never hide it.
- **Accessibility/contrast pass.** Fixed the `--color-cta`-resolves-to-teal-in-dark semantic bug; replaced failing white-on-mid-tone fills with dark `#1a1500` text on brand-color fills (level badges, rating chips); danger/coral now from `--color-accent-coral` (consistent across themes); secondary text bumped to `--color-text-dim` to clear 4.5:1 on surfaces.
- **Path safety.** Parent-relative refs; same-directory links use `../tools/...` so it works at `dxn.is/tools` with or without a trailing slash. No `vercel.json` change needed.
- **Reworked to a print take-away (2026-06-04).** A jobs-to-be-done pass concluded the real work happens off the site (a vendor meeting, with a team) and evals is qualitative judgment, so the interactive widget was the wrong modality. Removed the in-browser scorecard/worksheet (radio ratings, autosave, Save/Print/Email/Clear) and its JS; `tools.js` is now just theme/nav/accordion. The page is read-only and prints to a clean write-on-it worksheet via `@media print` (worksheet header with vendor/date lines, the scorecard with hand-mark Strong/Vague/Dodged boxes and note lines, a blank 10-row cases table). Print convention: `.tm-screen-only` / `.tm-print-only`, `#scorecard` and `#cases` are the only sections that print. README rewritten around the teach-page-plus-printable-take-away pattern. Ran through the compound-engineering pipeline: `docs/ideation/2026-06-03-tools-methods-delivery-ideation.md`, `docs/brainstorms/2026-06-04-evals-page-as-method-pattern-requirements.md`, `docs/plans/2026-06-04-001-refactor-evals-print-worksheet-plan.md`.
- **Open items.** Light theme and the print layout verified by code/CSS logic, not on-screen (the computer-use display has been unavailable). Not yet committed or deployed. Deferred: a copyable Google Doc companion, library-by-job reorganization, applying the pattern to a second method.
