...
const heroParticlesAmber = (p) => {
  ...
  // Flash timing constants — longer, warmer glow for amber
  const FLASH_RISE = 10;
  const FLASH_HOLD = 6;
  const FLASH_FALL = 30;
  ...
  class Cell {
    ...
    reset() {
      ...
      // Cell properties
      this.baseRadius = p.random(1.5, 2.5);   // larger, warmer
      this.maxSpeed = p.random(1.2, 2.5);     // slower, more deliberate

      // 25% of particles use rust accent color
    ...
    attractToMouse(mx, my) {
      ...
      if (distSq < influenceRadius * influenceRadius && distSq > 4) {
        ...
        const weaveOffset = Math.sin(angle * 2) * 18 * this.orbitDirection;
        ...
        if (dist > orbitRadius) {
          ...
          attractionStrength = 0.75 * pullIntensity;  // snappier pull
        } else {
          attractionStrength = -0.28 * (1 - dist / orbitRadius);
        }

        // Orbit strength - gentler for organic flow
        const orbitStrength = 0.75 * falloff;  // snappier orbit

        // Mouse velocity: slow = strong attraction, fast = weak
        const speedFactor = mouseSpeedSmooth < 2 ? 1.0
                          : mouseSpeedSmooth < 8 ? (1.0 - (mouseSpeedSmooth - 2) / 6 * 0.7)
                          : 0.1;
    ...
    show(isDarkMode) {
      ...
      // Base alpha with age fade — higher for BLEND mode (no ADD glow)
      let baseAlpha = isDarkMode ? 140 : 130;
      const fadeIn = 30;
      const fadeOut = 60;
      ...
      if (this.alignment > 0.15) {
        ...
        const weaveBrightness = isOver
          ? 1.0 + (1.0 - crossingFactor) * 0.3
          : 0.35 + crossingFactor * 0.65;
  ...
  p.draw = () => {
    ...
    if (isDarkMode) {
      ...
      p.background(245, 240, 232);  // warm off-white
    }
    ...
    // Amber uses BLEND mode (warm colors wash out with ADD)
    // No blend mode change needed — BLEND is default
    ...
    // Global breathing force (Lissajous drift)
      ...
      for (let i = 0; i < orbiting.length && lineCount < maxLines; i++) {
        for (let j = i + 1; j < orbiting.length && lineCount < maxLines; j++) {
          ...
          if (distSq < connDistSq) {
            ...
            const alpha = (1 - dist / connDist) * 40 * Math.min(orbiting[i].alignment, orbiting[j].alignment);
            p.stroke(212, 160, 66, alpha);
